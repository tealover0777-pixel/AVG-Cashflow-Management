...
export default function PageDealSummary({ t, isDark, dealId, DEALS = [], INVESTMENTS = [], CONTACTS = [], DIMENSIONS = [], FEES_DATA = [], SCHEDULES = [], USERS = [], LEDGER = [], setActivePage, selectedDistMemoId, setSelectedDistMemoId, investmentCollection = "investments", scheduleCollection = "paymentSchedules", tenantId, tenantFeatures = {} }) {
  ...
  const filteredPivotRows = useMemo(() => {
    ...
    return filtered.map(r => {
      const invKey = `${r.firstName}|||${r.lastName}`;
      if (invKey !== currentInv) {
        currentInv = invKey;
        currentIdx++;
      }
  ...
  const openEdit = (r) => {
    ...
    setModal({ 
      ...
      data: { 
        ...
        lag_month_offset: r.payment_lag_config ? (r.payment_lag_config.month_offset || 1) : (deal.payment_lag_config?.month_offset || 1),
      } 
    });
  };
  ...
  }, [scheduleColumnDefs, isDark, t, paymentStatusOpts, scheduleCollection]);

  const handleDistMemoBulkStatus = (status) => {
    if (!status || distMemoSel.size === 0) return;
    setConfirmAction({
      title: "Bulk Status Update",
      message: `Update status to "${status}" for ${distMemoSel.size} selected schedule(s)?`,
      onConfirm: async () => {
        try {
          ...
          setConfirmAction(null);
          showToast(`Statuses updated to ${status}.`, "success");
        } catch (err) {
          ...
          {modal.data.lag_override ? (
            ...
            <div style={{ fontSize: 12.5, color: t.textMuted, fontStyle: "italic", marginTop: deal.payment_lag_config?.enabled ? 4 : 0 }}>
              {deal.payment_lag_config?.enabled 
                ? `Inheriting Deal Default: ${
                    deal.payment_lag_config.type?.toUpperCase().replace(/ /g, "_") === 'SPECIFIC_DAY_OF_THE_FOLLOWING_MONTH'
                    ? `Day ${deal.payment_lag_config.specific_day} (Offset ${deal.payment_lag_config.month_offset}mo)`
                    : deal.payment_lag_config.type?.toUpperCase().includes("QUARTER")
                    ? `Quarter-End + ${deal.payment_lag_config.value} days`
                    : `${deal.payment_lag_config.value} ${deal.payment_lag_config.type?.charAt(0).toUpperCase() + deal.payment_lag_config.type?.slice(1).toLowerCase()}`
                  }`
