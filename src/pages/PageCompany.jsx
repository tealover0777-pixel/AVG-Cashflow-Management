            ...
            <div style={{ marginBottom: 28, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                ...
                <div style={{ display: "flex", gap: 12 }}>
                    <button onClick={handleSave} disabled={saving || !canUpdate} 
                        style={{ background: !canUpdate ? (isDark ? "rgba(255,255,255,0.05)" : "#E5E7EB") : t.accentGrad, color: !canUpdate ? t.textMuted : "#fff", border: "none", borderRadius: 12, padding: "12px 28px", fontSize: 14, fontWeight: 700, cursor: (!canUpdate || saving) ? "not-allowed" : "pointer", boxShadow: !canUpdate ? "none" : `0 6px 20px ${t.accentShadow}`, display: "flex", alignItems: "center", gap: 10, transition: "all 0.2s", opacity: !canUpdate ? 0.6 : 1 }}
                        onMouseEnter={e => { if (canUpdate && !saving) e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { if (canUpdate && !saving) e.currentTarget.style.transform = "translateY(0)"; }}>
                ...
                <div style={{ maxWidth: 800, margin: "0 auto" }}>
                    <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.surfaceBorder}`, padding: 32, boxShadow: t.tableShadow }}>
                        ...
                        <div style={{ display: "grid", gap: 24 }}>
                            <FF label="Organization Name" t={t}><FIn value={data.name} onChange={e => setData(p => ({ ...p, name: e.target.value }))} placeholder="Organization Name" t={t} disabled={!canUpdate} /></FF>
                            <FF label="Logo / Avatar" t={t}>
                                <div style={{ display: "grid", gap: 16 }}>
                                    ...
                                    >
                                        ...
                                        <div style={{ marginTop: 8 }}>
                                            <input type="file" id="tenant-logo-upload" accept="image/*" onChange={handlePhotoChange} style={{ display: "none" }} disabled={!canUpdate} />
                                            <label htmlFor={canUpdate ? "tenant-logo-upload" : undefined} style={{ background: !canUpdate ? (isDark ? "rgba(255,255,255,0.08)" : "#E5E7EB") : (logoUploading ? t.surfaceBorder : t.accentGrad), color: !canUpdate ? t.textMuted : "#fff", padding: "10px 22px", borderRadius: 10, fontSize: 13.5, fontWeight: 600, cursor: (!canUpdate || logoUploading) ? "not-allowed" : "pointer", display: "inline-block", opacity: logoUploading ? 0.6 : 1 }}>{data.logo ? "Upload New Logo" : "Upload Logo"}</label>
                                            {canUpdate && <p style={{ fontSize: 11, color: t.textMuted, marginTop: 12, fontWeight: 500 }}>Drag & drop or click to upload · PNG/JPEG · Max 2MB</p>}
                                        </div>
                                    </div>
                                        ...
                                        <div style={{ display: "grid", gap: 8 }}>
                                            ...
                                            <div style={{ position: "relative" }}>
                                                <select 
                                                    ...
                                                    disabled={!canUpdate}
                                                    style={{ width: "100%", padding: "10px 14px", borderRadius: 9, border: `1px solid ${t.border}`, background: isDark ? "rgba(255,255,255,0.05)" : "#fff", color: t.text, cursor: !canUpdate ? "not-allowed" : "pointer", fontSize: 13.5, appearance: "none", outline: "none", opacity: !canUpdate ? 0.6 : 1 }}
                                                >
                    ...
                    <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.surfaceBorder}`, padding: 32, boxShadow: t.tableShadow }}>
                        ...
                        <div style={{ display: "grid", gap: 20 }}>
                            <div style={{ display: "grid", gap: 16 }}>
                                ...
                                <FF label="Public Email" t={t}><FIn value={data.email} onChange={e => setData(s => ({ ...s, email: e.target.value }))} placeholder="e.g. contact@company.com" t={t} disabled={!canUpdate} /></FF>
                                ...
                                <FF label="Website / Home Page" t={t}><FIn value={data.home_page} onChange={e => setData(s => ({ ...s, home_page: e.target.value }))} placeholder="https://www.company.com" t={t} disabled={!canUpdate} /></FF>
                            </div>
                            ...
                            <FF label="OWNER" t={t}>
                                <div style={{ position: "relative" }}>
                                    <div onClick={canUpdate ? () => setShowOwnerSearch(!showOwnerSearch) : undefined} style={{ width: "100%", padding: "10px 14px", borderRadius: 9, border: `1px solid ${t.border}`, background: isDark ? "rgba(255,255,255,0.05)" : "#fff", color: t.text, cursor: canUpdate ? "pointer" : "not-allowed", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "space-between", opacity: canUpdate ? 1 : 0.6 }}>
                                        <span>{resolvedOwnerName}</span><span style={{ fontSize: 10, opacity: 0.5 }}>▼</span>
                                    </div>
                                    {showOwnerSearch && canUpdate && (
                            ...
                            <FF label={
                                ...
                                <div style={{ position: "relative" }}>
                                    <select 
                                        ...
                                        disabled={!canUpdate}
                                        style={{ width: "100%", padding: "10px 14px", borderRadius: 9, border: `1px solid ${t.border}`, background: isDark ? "rgba(255,255,255,0.05)" : "#fff", color: t.text, cursor: !canUpdate ? "not-allowed" : "pointer", fontSize: 14, outline: "none", appearance: "none", opacity: !canUpdate ? 0.6 : 1 }}
                                    >
                ...
                <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.surfaceBorder}`, padding: 32, boxShadow: t.tableShadow }}>
                    <div style={{ marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        {!isGlobalConsolidated && <div
                            onClick={canUpdate ? () => updES({ usePlatformEmail: !data.emailSetup.usePlatformEmail }) : undefined}
                            onMouseEnter={e => { if (canUpdate) e.currentTarget.style.borderColor = data.emailSetup.usePlatformEmail ? "rgba(59,130,246,0.5)" : t.accent; }}
                            onMouseLeave={e => { if (canUpdate) e.currentTarget.style.borderColor = data.emailSetup.usePlatformEmail ? "rgba(59,130,246,0.3)" : t.border; }}
                            style={{ 
                                display: "flex", alignItems: "center", gap: 16, cursor: canUpdate ? "pointer" : "not-allowed",
                        ...
                        <div>
                            <FF label="Delivery Method" t={t}>
                                <div style={{ display: "flex", gap: 8, background: isDark ? "rgba(255,255,255,0.05)" : "#F3F4F6", padding: 4, borderRadius: 10 }}>
                                    <button onClick={(!canUpdate || data.emailSetup.usePlatformEmail) ? undefined : () => updES({ method: "ESP" })}
                                        style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "none", fontSize: 13, fontWeight: 600, cursor: (!canUpdate || data.emailSetup.usePlatformEmail) ? "not-allowed" : "pointer", background: (data.emailSetup.usePlatformEmail ? platformEmailSetup?.method === "ESP" : data.emailSetup.method === "ESP") ? t.accentGrad : "transparent", color: (data.emailSetup.usePlatformEmail ? platformEmailSetup?.method === "ESP" : data.emailSetup.method === "ESP") ? "#fff" : t.textMuted, position: "relative" }}>
                                    ...
                                    <button onClick={(!canUpdate || data.emailSetup.usePlatformEmail) ? undefined : () => updES({ method: "SMTP" })}
                            ...
                            <div style={{ marginTop: 40, padding: 24, borderRadius: 16, border: `1px solid ${t.border}`, background: isDark ? "rgba(255,255,255,0.02)" : "#FAFAF9" }}>
                                ...
                                <button
                                    ...
                                    style={{ width: "100%", padding: "10px", borderRadius: 8, background: (!canUpdate) ? (isDark ? "rgba(255,255,255,0.04)" : "#E5E7EB") : (isDark ? "rgba(255,255,255,0.08)" : "#fff"), border: `1px solid ${t.border}`, color: (!canUpdate) ? t.textMuted : t.text, fontSize: 13, fontWeight: 600, cursor: (testingEmail || saving || !canUpdate) ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "all 0.2s" }}
                                    onMouseEnter={e => { if (canUpdate && !testingEmail && !saving) e.currentTarget.style.background = isDark ? "rgba(255,255,255,0.12)" : "#F3F4F6"; }}
                                    onMouseLeave={e => { if (canUpdate && !testingEmail && !saving) e.currentTarget.style.background = isDark ? "rgba(255,255,255,0.08)" : "#fff"; }}
                                >
                        ...
                        {!data.emailSetup.usePlatformEmail && (
                            ...
                            {(data.emailSetup.usePlatformEmail ? platformEmailSetup?.method === "ESP" : data.emailSetup.method === "ESP") && (
                                <>
                                    ...
                                    <FF label="Provider" t={t}>
                                        <div style={{ position: "relative" }}>
                                            <select 
                                                ...
                                                disabled={data.emailSetup.usePlatformEmail || !canUpdate}
                                                style={{ width: "100%", padding: "10px 14px", borderRadius: 9, border: `1px solid ${t.border}`, background: isDark ? "rgba(255,255,255,0.05)" : "#fff", color: t.text, cursor: (data.emailSetup.usePlatformEmail || !canUpdate) ? "not-allowed" : "pointer", fontSize: 14, outline: "none", appearance: "none", opacity: (data.emailSetup.usePlatformEmail || !canUpdate) ? 0.6 : 1 }}
                                            >
                                    ...
                                    <FF label="API Key 🔐" t={t}><FIn type="password" value={data.emailSetup.usePlatformEmail ? "••••••••••••••••" : data.emailSetup.api.apiKey} onChange={e => updAPI({ apiKey: e.target.value })} placeholder="Your API key" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} /></FF>
                                    {(data.emailSetup.usePlatformEmail ? platformEmailSetup?.api?.provider === "Mailgun" : data.emailSetup.api.provider === "Mailgun") && (
                                        <FF label="Domain" t={t}><FIn value={data.emailSetup.usePlatformEmail ? (platformEmailSetup?.api?.domain || "") : data.emailSetup.api.domain} onChange={e => updAPI({ domain: e.target.value })} placeholder="mg.yourdomain.com" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} /></FF>
                                    )}
                                    {(data.emailSetup.usePlatformEmail ? platformEmailSetup?.api?.provider === "Amazon SES" : data.emailSetup.api.provider === "Amazon SES") && (
                                        <FF label="Region" t={t}><FIn value={data.emailSetup.usePlatformEmail ? (platformEmailSetup?.api?.region || "") : data.emailSetup.api.region} onChange={e => updAPI({ region: e.target.value })} placeholder="us-east-1" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} /></FF>
                                    )}
                                    <FF label="API Base URL (Optional)" t={t}><FIn value={data.emailSetup.usePlatformEmail ? (platformEmailSetup?.api?.baseUrl || "") : data.emailSetup.api.baseUrl} onChange={e => updAPI({ baseUrl: e.target.value })} placeholder="https://api.provider.com" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} /></FF>
                                </>
                            )}
                            ...
                            {(data.emailSetup.usePlatformEmail ? platformEmailSetup?.method === "SMTP" : data.emailSetup.method === "SMTP") && (
                                <>
                                    ...
                                    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12 }}>
                                        <FF label="SMTP Host" t={t}><FIn value={data.emailSetup.usePlatformEmail ? (platformEmailSetup?.smtp?.host || "") : data.emailSetup.smtp.host} onChange={e => updSMTP({ host: e.target.value })} placeholder="smtp.provider.com" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} /></FF>
                                        <FF label="Port" t={t}><FIn value={data.emailSetup.usePlatformEmail ? (platformEmailSetup?.smtp?.port || "") : data.emailSetup.smtp.port} onChange={e => updSMTP({ port: e.target.value })} placeholder="587" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} /></FF>
                                    </div>
                                    <FF label="SMTP Username" t={t}><FIn value={data.emailSetup.usePlatformEmail ? (platformEmailSetup?.smtp?.user || "") : data.emailSetup.smtp.user} onChange={e => updSMTP({ user: e.target.value })} placeholder="username@provider.com" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} /></FF>
                                    
                                    <div style={{ marginBottom: 16 }}>
                                        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: (data.emailSetup.usePlatformEmail || !canUpdate) ? "not-allowed" : "pointer", marginBottom: 12, opacity: (data.emailSetup.usePlatformEmail || !canUpdate) ? 0.6 : 1 }}>
                                            <input type="checkbox" checked={data.emailSetup.usePlatformEmail ? !!platformEmailSetup?.smtp?.has2FA : data.emailSetup.smtp.has2FA} onChange={e => updSMTP({ has2FA: e.target.checked })} style={{ width: 16, height: 16, accentColor: t.accent }} disabled={data.emailSetup.usePlatformEmail || !canUpdate} />
                                        ...
                                        {(data.emailSetup.usePlatformEmail ? platformEmailSetup?.smtp?.has2FA : data.emailSetup.smtp.has2FA) ? (
                                            <FF label="App Password 🔐" t={t}>
                                                <FIn type="password" value={data.emailSetup.usePlatformEmail ? "••••••••••••••••" : data.emailSetup.smtp.pass} onChange={e => updSMTP({ pass: e.target.value })} placeholder="16-character app password" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} />
                                            ...
                                            <FF label="SMTP Password 🔐" t={t}>
                                                <FIn type="password" value={data.emailSetup.usePlatformEmail ? "••••••••••••••••" : data.emailSetup.smtp.pass} onChange={e => updSMTP({ pass: e.target.value })} placeholder="Your password" t={t} disabled={data.emailSetup.usePlatformEmail || !canUpdate} />
                                            </FF>
                                        )}
                                    </div>

                                    <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: (data.emailSetup.usePlatformEmail || !canUpdate) ? "not-allowed" : "pointer", marginTop: 4, opacity: (data.emailSetup.usePlatformEmail || !canUpdate) ? 0.6 : 1 }}>
                                        <input type="checkbox" checked={data.emailSetup.usePlatformEmail ? !!platformEmailSetup?.smtp?.secure : data.emailSetup.smtp.secure} onChange={e => updSMTP({ secure: e.target.checked })} style={{ width: 16, height: 16, accentColor: t.accent }} disabled={data.emailSetup.usePlatformEmail || !canUpdate} />
            ...
            {activeTab === "ACH" && (
                <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.surfaceBorder}`, padding: 32, boxShadow: t.tableShadow }}>
                    <div style={{ marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: !canUpdate ? "not-allowed" : "pointer", background: isDark ? "rgba(255,255,255,0.05)" : "#F3F4F6", padding: "8px 16px", borderRadius: 10, border: `1px solid ${data.achSetup.enabled ? t.accent : t.border}`, opacity: !canUpdate ? 0.6 : 1 }}>
                            ...
                            <input type="checkbox" checked={data.achSetup.enabled} onChange={e => updACH({ enabled: e.target.checked })} style={{ width: 18, height: 18, accentColor: t.accent }} disabled={!canUpdate} />
                        </label>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, opacity: data.achSetup.enabled ? 1 : 0.6, pointerEvents: (data.achSetup.enabled) ? "auto" : "none", transition: "opacity 0.2s" }}>
                        <div style={{ display: "grid", gap: 20 }}>
                            ...
                            <FF label="Originator Name" t={t}><FIn value={data.achSetup.originatorName} onChange={e => updACH({ originatorName: e.target.value })} placeholder="Company Name" t={t} disabled={!canUpdate} /></FF>
