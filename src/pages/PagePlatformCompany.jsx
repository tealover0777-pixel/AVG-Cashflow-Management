...
export default function PagePlatformCompany({ t, isDark, USERS = [], CONTACTS = [] }) {
    ...
    React.useEffect(() => {
        if (!data.emailSetup.timeZone && (data.state || data.zip)) {
            ...
            if (resolved) {
                ...
                setDoc(doc(db, "platform_config", "company"), { emailSetup: { timeZone: resolved } }, { merge: true }).catch(() => {});
            }
        }
    }, [data.state, data.zip]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.error("Save platform company error:", err);
                ...
                <div style={{ maxWidth: 800, margin: "0 auto" }}>
                    <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.surfaceBorder}`, padding: 32, boxShadow: t.tableShadow }}>
                        <div style={{ marginBottom: 24 }}>
                            <h3 style={{ fontSize: 17, fontWeight: 700, color: isDark ? "#fff" : "#1C1917", marginBottom: 6 }}>Platform Branding</h3>
                ...
                <div style={{ maxWidth: 800, margin: "0 auto" }}>
                    <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.surfaceBorder}`, padding: 32, boxShadow: t.tableShadow }}>
                        <div style={{ marginBottom: 24 }}>
                            ...
                            <p style={{ fontSize: 12.5, color: t.textMuted, lineHeight: 1.5 }}>Platform company contact and location details.</p>
                        </div>
