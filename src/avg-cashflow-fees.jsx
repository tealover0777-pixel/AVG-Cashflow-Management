...
const methodCfg = (method, isDark) => ({
  Percentage: { bg: isDark ? "rgba(96,165,250,0.15)" : "#EFF6FF", color: isDark ? "#60A5FA" : "#2563EB", border: isDark ? "rgba(96,165,250,0.3)" : "#BFDBFE" },
  Flat:       { bg: isDark ? "rgba(167,139,250,0.15)" : "#F5F3FF", color: isDark ? "#A78BFA" : "#7C3AED", border: isDark ? "rgba(167,139,250,0.3)" : "#DDD6FE" },
})[method] || { bg: "transparent", color: "#888", border: "#ccc" };
...
export default function App() {
  ...
  const statData = [
    ...
    { label: "Percentage-Based", value: fees.filter(f => f.method === "Percentage").length, accent: isDark ? "#34D399" : "#059669", bg: isDark ? "rgba(52,211,153,0.08)" : "#ECFDF5", border: isDark ? "rgba(52,211,153,0.15)" : "#A7F3D0" },
    { label: "Flat Amount", value: fees.filter(f => f.method === "Flat").length, accent: isDark ? "#A78BFA" : "#7C3AED", bg: isDark ? "rgba(167,139,250,0.08)" : "#F5F3FF", border: isDark ? "rgba(167,139,250,0.15)" : "#DDD6FE" },
    { label: "Recurring", value: fees.filter(f => f.frequency !== "One-time" && f.frequency !== "Per occurrence").length, accent: isDark ? "#FBBF24" : "#D97706", bg: isDark ? "rgba(251,191,36,0.08)" : "#FFFBEB", border: isDark ? "rgba(251,191,36,0.15)" : "#FDE68A" },
  ];
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ height: 58, borderBottom: `1px solid ${t.topbarBorder}`, background: t.topbar, backdropFilter: t.glass ? "blur(10px)" : "none", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: t.breadcrumb }}>
            ...
            <span style={{ color: t.breadcrumbActive, fontWeight: 500 }}>Fees</span>
          </div>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "32px 36px" }}>
          ...
          <div style={{ marginBottom: 28, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
            <div>
              <h1 style={{ fontFamily: t.titleFont, fontWeight: isDark ? 800 : 700, fontSize: isDark ? 30 : 38, color: isDark ? "#fff" : "#1C1917", letterSpacing: isDark ? "-1px" : "-1.5px", lineHeight: 1, marginBottom: 6 }}>Fees</h1>
              <p style={{ fontSize: 13.5, color: t.textMuted }}>Define and manage fee structures</p>
            </div>
            <button className="primary-btn" style={{ background: t.accentGrad, color: "#fff", padding: "11px 22px", borderRadius: 11, fontSize: 13.5, fontWeight: 600, boxShadow: `0 4px 16px ${t.accentShadow}`, display: "flex", alignItems: "center", gap: 7 }}>
              <span style={{ fontSize: 18, lineHeight: 1 }}>+</span> New Fee
            </button>
          </div>
          ...
          <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.surfaceBorder}`, overflow: "hidden", backdropFilter: t.glass ? "blur(20px)" : "none", boxShadow: t.tableShadow }}>
            <div style={{ display: "grid", gridTemplateColumns: "100px 1fr 130px 120px 140px 1fr 90px", padding: "12px 22px", background: t.tableHeader, borderBottom: `1px solid ${t.surfaceBorder}` }}>
              {["ID", "NAME", "METHOD", "RATE", "FREQUENCY", "DESCRIPTION", "ACTIONS"].map(col => (
            ...
            {fees.map((f, i) => {
              ...
              return (
                <div key={f.id} className="data-row" onMouseEnter={() => setHoveredRow(f.id)} onMouseLeave={() => setHoveredRow(null)}
                  ...
                  <div style={{ fontFamily: t.monoFont, fontSize: 12.5, fontWeight: 700, color: isDark ? "#60A5FA" : "#4F46E5" }}>{f.rate}</div>
